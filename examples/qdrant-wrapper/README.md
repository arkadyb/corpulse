# Qdrant Wrapper Demo

Demonstrates corpulse's zero-instrumentation Qdrant wrapper. Wraps a `QdrantClient` so that every `query_points()` call automatically logs retrievals — no manual `log_retrieval()` needed.

## What it does

1. Creates a Qdrant collection with 10 sample documents
2. Wraps the client: `QdrantCorpulseClient(client, corp)`
3. Runs 25 queries — retrievals tracked automatically
4. Simulates user engagement and a stale source update
5. Prints a full corpus health report with cleanup recommendations

## Prerequisites

```bash
pip install "git+https://github.com/arkadyb/corpulse[qdrant]"
```

## Running Qdrant

The demo uses Qdrant's **in-memory mode** by default — no server needed.

To use a real Qdrant server instead:

```bash
# Start Qdrant via Docker
docker run -p 6333:6333 qdrant/qdrant
```

Then edit `demo.py` line 36:
```python
QDRANT_LOCATION = "localhost"   # instead of ":memory:"
```

## Run

```bash
python demo.py
```

## Sample Output

```
============================================================
CORPULSE  ×  QDRANT  —  Sample Application
============================================================

✓ Inserted 10 documents into 'knowledge_base' collection

✓ Wrapped QdrantClient with QdrantCorpulseClient
  Every query_points() call now auto-logs retrievals.

------------------------------------------------------------
Running sample queries...
------------------------------------------------------------
  Q: "how do I get started?"
     → getting-started.md, security-overview.md, internal-draft.md
  Q: "API authentication docs"
     → api-reference-v2.md, changelog.md, troubleshooting.md
  Q: "environment setup steps"
     → setup-guide.md, setup-guide-copy.md, internal-draft.md
  Q: "fix connection timeout error"
     → troubleshooting.md, setup-guide.md, setup-guide-copy.md
  Q: "security compliance"
     → security-overview.md, getting-started.md, internal-draft.md

✓ Ran 25 queries — all automatically tracked

Simulating user engagement...
✓ Engagement and source updates logged

============================================================
CORPUS HEALTH REPORT
============================================================

  corpulse — Corpus Health Report
  10 documents · last 30 days · ⚠ corpus bloat detected (50% noise est.)
  Document                             Retrieved   Engagement  Status
  ──────────────────────────────────────────────────────────────────────
  internal-draft.md                           17           0%  ◌  low eng.
  setup-guide.md                              11           0%  ◌  low eng.
  setup-guide-copy.md                         11           0%  ◌  low eng.
  getting-started.md                          10         150%  ✓  healthy
  security-overview.md                        10           0%  ◌  low eng.
  troubleshooting.md                           8          25%  ✓  healthy
  api-reference-v2.md                          4         375%  ✓  healthy
  changelog.md                                 4           0%  ◌  low eng.
  api-reference-v1.md                          0            —  👻 ghost
  pricing-2023.md                              0            —  👻 ghost

  👻 ghosts: 2  💀 obsolete: 1  ⚠ duplicates: 2  🕓 stale: 0
  Run corpulse.cleanup_report() for a prioritised action list.

------------------------------------------------------------
CLEANUP RECOMMENDATIONS
------------------------------------------------------------

────────────────────────────────────────────────────────────
  corpulse — Cleanup Report
────────────────────────────────────────────────────────────
  Total documents : 10
  Noise estimate  : 50%
  ⚠  Consider pruning ~5 low-signal documents.

  👻  GHOSTS  (2 docs — never retrieved in 30d)
      · api-reference-v1.md
      · pricing-2023.md

  💀  OBSOLETE  (1 docs)
      · api-reference-v1.md  →  superseded by api-reference-v2.md

  🔁  RE-CHUNK CANDIDATES  (4 docs — high retrieval, low engagement)
      · internal-draft.md  (17 retrievals, 0% engagement)
      · setup-guide.md  (11 retrievals, 0% engagement)
      · setup-guide-copy.md  (11 retrievals, 0% engagement)
      · security-overview.md  (10 retrievals, 0% engagement)

------------------------------------------------------------
DETAILED FINDINGS
------------------------------------------------------------

Ghost documents (2):
  · api-reference-v1.md — never retrieved, safe to remove
  · pricing-2023.md — never retrieved, safe to remove

Near-duplicate pairs (1):
  · setup-guide.md  ↔  setup-guide-copy.md  (similarity: 0.99)

Obsolete versions (1):
  · api-reference-v1.md  → superseded by api-reference-v2.md

Stale embeddings (0):

Overall health score:
  total_docs            : 10
  ghosts                : 2
  obsolete              : 1
  stale                 : 0
  duplicates            : 2
  noise_estimate        : 0.5
  bloat_warning         : True
  recommendation        : Consider pruning ~5 low-signal documents.

============================================================
✓ Demo complete. Database written to ./qdrant_demo.db
============================================================
```
